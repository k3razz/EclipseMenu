...
public static class PlayerControl_FixedUpdate
{
    public static void Postfix(PlayerControl __instance)
    {
        ...
        {
            EclipseCheats.NoKillCdCheat(__instance);
        }
...
public static class PlayerControl_CmdCheckMurder
{
    ...
    public static bool Prefix(PlayerControl __instance, PlayerControl target)
    {
        ...
        if (!CheatToggles.killAnyone && !CheatToggles.zeroKillCd && !Utils.isVanished(__instance.Data) &&
            !Utils.isMeeting &&
            (EclipsePPMCheats.oldRole == null ||
             Utils.getBehaviourByRoleType((AmongUs.GameOptions.RoleTypes)EclipsePPMCheats.oldRole).IsImpostor))
            return true;
