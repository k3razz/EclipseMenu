...
public static class Constants_GetPlatformData
{
    ...
    {
        if (Utils.StringToPlatformType(EclipseMenu.spoofPlatform.Value, out Platforms? platformType))
        {
...
public static class ChatBubble_SetName
{
	...
	{
        EclipseESP.ChatNametags(__instance);
    }
}
...
public static class SystemInfo_deviceUniqueIdentifier_Getter
{
    ...
    {
        if (!EclipseMenu.spoofDeviceId.Value) return;
...
{
    // Postfix patch of VersionShower.Start to show EclipseMenu version
    public static void Postfix(VersionShower __instance)
    {
        if (EclipseMenu.inStealthMode || EclipseMenu.isPanicked) return;

        if (EclipseMenu.supportedAU.Contains(Application.version)) // Checks if Among Us version is supported
        {
            __instance.text.text =  $"EclipseMenu v{EclipseMenu.EclipseVersion} (v{Application.version})"; // Supported
        }
        else
        {
            __instance.text.text =  $"EclipseMenu v{EclipseMenu.EclipseVersion} (<color=red>v{Application.version}</color>)"; // Unsupported
        }
    }
}
...
{
    // Postfix patch of PingTracker.Update to show EclipseMenu authors and colored ping text
...
public static class Mushroom_FixedUpdate
{
    ...
    {
        EclipseESP.SporeCloudVision(__instance);
    }
}
