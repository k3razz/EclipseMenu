...
public class MenuUI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(Utils.StringToKeycode(EclipseMenu.menuKeybind.Value)))
        {
            // Enable or disable GUI with DELETE key
            isGUIActive = !isGUIActive;

            if (EclipseMenu.menuOpenOnMouse.Value)
            {
        ...
        if (CheatToggles.stealthMode != EclipseMenu.inStealthMode)
        {
            EclipseMenu.inStealthMode = CheatToggles.stealthMode;
        ...
        if (stamp) stamp.enabled = !(EclipseMenu.inStealthMode || EclipseMenu.isPanicked);
        ...
        {
            EclipseMenu.Plugin.Config.Reload();
        ...
        if(!Utils.isShip)
        {
            ...
            EclipseCheats.StopShipAnimCheats();
        }
    ...
    public void OnGUI()
    {
        if (!isGUIActive || EclipseMenu.isPanicked) return;

        InitStyles();

        UIHelpers.ApplyUIColor();

        _windowRect = GUI.Window((int)WindowId.MenuUI, _windowRect, (GUI.WindowFunction)WindowFunction, "EclipseMenu v" + EclipseMenu.EclipseVersion);
    }
